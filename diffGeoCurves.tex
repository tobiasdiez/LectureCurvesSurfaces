...
\begin{document}
    ...
    \part{Curves}\label{Part:Curves}
        ...
        \section{Parameterized curves}
            ...
            The regularity condition ensures that the curve has a well-defined tangent vector at every point, which is crucial for many applications in differential geometry.
            ...
            \end{proposition}
            A curve with unit-speed is also called an \emphDef{arc-length parameterization} of the curve, since the parameter $t$ directly measures the length along the curve from some starting point.
	...
	\section{Vector fields and Tangent space}\label{sec:Vector fields and Tangent space}
		...
		\begin{definition}[$T_pf$]
			...
			\begin{equation*}
				\begin{aligned}
					T_pf(X) := \difFracAt{}{t}{t=0} f\big(c(t)\big) \in\mathbb{R}
				\end{aligned}
			\end{equation*}
        ...
        \begin{proof}
            \begin{equation*}
                \begin{aligned}
                    ...
                    &= \int\limits_{a}^{b}\cfrac{df\big(c(t)\big)}{d\big(c(t)\big)}\big(\dot{c}(t)\big)dt = \int\limits_{a}^{b}\difFrac{}{t}f\big(c(t)\big)dt = f\big(c(t)\big)\bigg|_a^b
                \end{aligned}
            \end{equation*}
        \end{proof}
	...
	\section{Gauss map and shape operator}\label{Sec:Gauss map and shape operator}
		...
		\begin{example}\label{Example: the guass map of sphere}
			...
			\begin{equation*}
				\begin{aligned}
					\mathcal{S}_p(v) = -T_pn(v) = -\difFracAt{}{t}{t=0} n\big( c(t) \big) = -\difFracAt{}{t}{t=0} \cfrac{c(t)}{R} = -\cfrac{v}{R}
				\end{aligned}
			\end{equation*}
        ...
        \section{Gaussian curvature}
            ...
            \begin{example}[Pushforward of tangent vectors]
                ...
                \begin{equation*}
                    \begin{aligned}
                        w = \difFracAt{}{t}{t=0} F(c(t)) = T_pF\bigg(\difFracAt{}{t}{t=0} c(t)\bigg) = T_pF(v)
                    \end{aligned}
                \end{equation*}
            ...
            \begin{proof}
                ...
                \begin{equation*}
                    \begin{aligned}
                        L(c) = L(F\circ c) = \int_0^1\bigg|\bigg|\difFrac{}{t} F\big(c(t)\big)\bigg|\bigg|dt
                    \end{aligned}
                \end{equation*}
            \end{proof}
        ...
        \section{Geodesics: parallel transport}
            ...
            According to the definition, $X(t) = \dot{c}(t)$ is a vector field along $c$. The derivative of $X$, is a map $\difFrac{}{t} X: I\rightarrow\mathbb{R}^3$, but $\difFrac{}{t} X$ is usually not tangent to $S$. The idea to handle this is to decompose $\difFrac{}{t} X$ into part that is tangent to $S$ and another part that is normal.
